...
class Absen extends CI_Controller {
    ...
    public function data() {
        ...
        $data['sudah'] = $this->db->query("select*from absensi,siswa where absensi.IDSiswa=siswa.no_peserta AND absensi.KodeSoal='".$uri4."' AND  siswa.kelas='".$uri3."' order by siswa.IDSiswa ASC");
        } else {
        $data['soa'] = $this->db->query('select*from siswa where siswa.kelas="'.$uri3.'" AND NOT EXISTS(SELECT * FROM absensi WHERE absensi.KodeSoal="'.$uri4.'" AND siswa.no_peserta = absensi.IDSiswa)');
        $data['sudah'] = $this->db->query("select*from absensi,siswa where absensi.IDSiswa=siswa.no_peserta AND absensi.KodeSoal='".$uri4."' AND  siswa.kelas='".$uri3."' order by siswa.IDSiswa ASC");
        }
